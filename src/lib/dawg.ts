...
const dedupe = <K, V>(
  ...
  const edges =
    ...
    Array.from(map.entries())
      .map(([key, child]): [K, number] => [key, dedupe(data, child, memo)])
      .sort();
...
// DAWG stands for "deterministic acyclic word graph", which is similar to a
// trie. Unlike a trie, the edges of a DAWG can form an arbitrary directed
// acyclic graph. Compress minimizes the number of nodes in this graph.
...
class DAWG<K, V> {
  ...
  static deserialize<K, V>(serialized: Serialized<K, V>): DAWG<K, V> {
    const dawg = new DAWG<K, V>();
    dawg.data.edges = serialized[0].map(x => x && new Map(x));
    dawg.data.nodes = serialized[1].map(x => x && new Set(x));
    return dawg;
  }
}
