...
type Lambda =
  | {repr?: string, type: 'binary'; base: Lambda[]; op: Binary}
  | {repr?: string, type: 'custom'; base: Lambda[]; op: string}
  | {repr?: string, type: 'single'; base: string}
  | {repr?: string, type: 'unary'; base: Lambda; op: Unary};
...
const stringify = (x: Lambda): string => {
  return x.repr || (x.repr = stringify_base(x));
}

const stringify_base = (x: Lambda): string => {
  switch (x.type) {
    case 'binary': {
      const op = x.op;
      const {commutes, precedence} = kBinaryData[op];
      const spacer = op === '.' ? op : ` ${op} `;
      const pieces = x.base.map(y => stringify_wrap(y, precedence));
      return (commutes ? pieces.sort() : pieces).join(spacer);
    }
    case 'custom':
    ...
    case 'unary': {
      ...
      const base = stringify_wrap(x.base, precedence);
      return op === '!' ? `R[${base}]` : `${op}${base}`;
    }
  }
};

const stringify_wrap = (x: Lambda, context: number): string => {
  const parens =
    (x.type === 'binary' && kBinaryData[x.op].precedence >= context) ||
    (x.type === 'unary' && kUnaryData[x.op].precedence >= context);
  return parens ? `(${stringify(x)})` : stringify(x);
};
