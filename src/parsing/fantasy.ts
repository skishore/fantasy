...
import {arithmetic} from '../../test/parsing/generator';
...
const bench = (n: number) => {
  ...
  }
  {
    ...
    const time = Date.now();
    for (let i = 0; i < n; i++) {
      assert(nonnull(P.parse(grammar, '(1+2)*3-4+5*6')).some === 35);
    }
  ...
  }
  {
    const time = Date.now();
    for (let i = 0; i < n; i++) {
      nonnull(P.parse(grammar, 'meri bacche ko pani chahie'));
    }
  ...
  {
    const base = P.parse(grammar, 'do accha acche larki ko pani chahie');
