...
const build = (template: string, optional: boolean[]): Template => {
  const slots = optional.map((x, i) => ({index: i, optional: x}));
  return new Template(template).index(optional.length, slots);
}
...
const template: Test = {
  ...
  merge_concatenates_repeated_list_key: () => {
    const pattern = '{...$0, ...$1}';
    const template = build(pattern, optional(2));
  ...
  split_assigns_value_to_variable: () => {
    const pattern = '$0';
    const template = build(pattern, optional(1));
  ...
  split_assigns_to_nested_key_value_pair: () => {
    ...
    const template = build(pattern, [false, true]);
  ...
  split_assigns_to_variables_within_dict: () => {
    ...
    const template = build(pattern, optional(5));
  ...
  split_generates_multiple_hypotheses_for_dict_and_variable: () => {
    ...
    Test.assert_eq(build(pattern, optional(2)).split(value),
                   [{0: 42, 1: {str: 'is'}},
                    {0: null, 1: {num: 42, str: 'is'}}]);
    Test.assert_eq(build(pattern, [true, false]).split(value),
                   [{0: 42, 1: {str: 'is'}},
                    {0: null, 1: {num: 42, str: 'is'}}]);
    Test.assert_eq(build(pattern, [false, true]).split(value),
                   [{0: 42, 1: {str: 'is'}}]);
    Test.assert_eq(build(pattern, required(2)).split(value),
                   [{0: 42, 1: {str: 'is'}}]);
  },
  ...
  split_generates_one_hypothesis_for_list_of_singletons: () => {
    ...
    Test.assert_eq(xs.map((x) => build(pattern, x).split([4, 5, 6])),
                   [[], [], [], []]);
    Test.assert_eq(xs.map((x) => build(pattern, x).split([4, 5])),
                   [[{0: 4, 1: 5}], [{0: 4, 1: 5}],
                    [{0: 4, 1: 5}], [{0: 4, 1: 5}]]);
    Test.assert_eq(xs.map((x) => build(pattern, x).split([4])),
                   [[{0: 4, 1: null}, {0: null, 1: 4}],
                    [{0: null, 1: 4}], [{0: 4, 1: null}], []]);
    Test.assert_eq(xs.map((x) => build(pattern, x).split([])),
                   [[{0: null, 1: null}], [], [], []]);
  },
  ...
  split_fails_on_null_required_variable: () => {
    const pattern = '$0';
    const template = build(pattern, required(1));
  ...
  split_fails_when_required_dict_key_is_missing: () => {
    const pattern = '{num: $0}';
    Test.assert_eq(build(pattern, optional(1)).split({}),
                   [{0: null}]);
    Test.assert_eq(build(pattern, required(1)).split({}), []);
  },
  split_fails_without_enough_list_keys: () => {
    const pattern = '[...$0, ...$1, ...$2]';
    const value = [42, 'is']
    Test.assert_eq(build(pattern, optional(3)).split(value),
    ...
    Test.assert_eq(build(pattern, [false, true, true])
                               .split(value),
                   [{0: [42, 'is'], 1: null, 2: null},
                    {0: [42], 1: ['is'], 2: null},
                    {0: [42], 1: null, 2: ['is']}]);
    Test.assert_eq(build(pattern, [false, false, true])
                               .split(value),
                   [{0: [42], 1: ['is'], 2: null}]);
    Test.assert_eq(build(pattern, required(3)).split(value), []);
  },
  parse_fails_on_unquoted_string_literal: () => {
    Test.assert_error(() => build('failed', []), 'At line 1, column 1');
  },
  parse_fails_on_malformatted_dict: () => {
    Test.assert_error(() => build('{num; 42}', []), 'At line 1, column 5');
  },
  parse_fails_on_malformatted_list: () => {
    Test.assert_error(() => build('["a" "b"]', []), 'At line 1, column 6');
  },
  parse_fails_when_variable_is_out_of_bounds: () => {
    const pattern = '$0';
    Test.assert_error(() => build(pattern, []), 'Index out of bounds: $0');
  },
