...
const value: Test = {
  boolean_template_works: () => {
    const template = Value.template('false');
  ...
  number_template_works: () => {
    const template = Value.template('17.5');
  ...
  string_template_works: () => {
    const template = Value.template('"1000"');
  ...
  dict_template_works: () => {
    const template = Value.template('{num: 17, str: "is", bool: false}');
  ...
  list_template_works: () => {
    const template = Value.template('[17, "is", false]');
  ...
  variable_template_works: () => {
    const template = Value.template('$2');
  ...
  dict_with_variables_works: () => {
    const template = Value.template('{num: $0, bool: $2}');
  ...
  dict_with_spreads_works: () => {
    const template = Value.template('{num: $0, ...$1, bool: $2}');
  ...
  parse_fails_on_unquoted_string_literal: () => {
    Test.assert_error(() => Value.template('failed'), 'At line 1, column 1');
  },
  parse_fails_on_malformatted_dict: () => {
    Test.assert_error(() => Value.template('{num; 42}'), 'At line 1, column 5');
  },
  parse_fails_on_malformatted_list: () => {
    Test.assert_error(() => Value.template('["a" "b"]'), 'At line 1, column 6');
  },
  parse_handles_underscore: () => {
    const t = Value.template('{abc_de_f: false}');
    Test.assert_eq(t.merge([]), {abc_de_f: false});
  },
  parse_handles_whitespace: () => {
    const t = Value.template(' { x : [ true , 2 , "3" , ...$0 , $1 ] } ');
