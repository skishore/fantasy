...
const assert_error = (fn: () => void, message: string): void => {
  try {
    fn();
  } catch (error) {
...
const check_eq = <T>(a: T, b: T): boolean => {
  if (!a || !b) {
    ...
    return Object.keys(a).every((x) => check_eq((a as any)[x], (b as any)[x]));
  } else {
    return a === b;
  }
}

const print_error = <T>(name: string, a: T, b: T): string => {
  const br = '\n      ';
  return `${name} failed:${br}  Actual: ${a}${br}Expected: ${b}`;
}

// Returns a Unix status code: 0 for success, 1 for failure.
const run = (tests: {[module: string]: Test}): number => {
